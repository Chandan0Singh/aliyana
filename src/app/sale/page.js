  ...
  return (
    <div className='flex '>
      ...
      <div className=" bg-white px-6 py-10 ">
        <h1 className="text-3xl font-bold text-gray-800 mb-8">🛍️ Shop All Bags</h1>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 h-[90vh] overflow-auto pr-[2rem] custom-scrollbar h-[90vh] overflow-auto pr-[2rem] custom-scrollbar">
          {bagsData.map((bag) => {
            ...
            return (
              <div key={bag.id} className="bg-gray-100 rounded-xl shadow p-4 hover:shadow-lg transition-all duration-300">
                ...
                {bag.sale ? (
                  <div className="mt-1">
                    <p className="text-sm text-gray-500 line-through">₹{bag.price}</p>
                    <p className="text-red-600 font-semibold">₹{discountedPrice} <span className="text-xs text-green-600 ml-2">{bag.sale} off</span></p>
                  </div>
                ) : (
