...
const NewArrivalPage = () => {
    const newArrivals = bagsData.filter(bag =>
        bag.arrivalDate && isNewArrival(bag.arrivalDate)
    );
    ...
    return (
        <div className='flex '>
            ...
            <div className="p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 h-[90vh] overflow-auto pr-[2rem] custom-scrollbar w-full">
                    {newArrivals.map(bag => (
                        <div key={bag.id} className="bg-white rounded shadow p-4">
                            <img src={bag.image} alt={bag.name} className="h-60 object-cover w-full rounded mb-4" />
